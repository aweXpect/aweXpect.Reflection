		...
		public sealed class Tests
		{
			...
			public async Task ShouldFilterForTypesWithName()
			{
				...
				await That(events.GetDescription())
					.IsEqualTo("events with name equal to \"SomeEventToVerifyTheNameOfIt\" in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportAsPrefix()
			{
				...
				await That(events).HasSingle().Which.IsEqualTo(ExpectedEventInfo());
				await That(events.GetDescription())
					.IsEqualTo("events with name starting with \"SomeEventToVerifyTheNameOf\" in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportAsRegex()
			{
				...
				await That(events).HasSingle().Which.IsEqualTo(ExpectedEventInfo());
				await That(events.GetDescription())
					.IsEqualTo("events with name matching regex \"[a-zA-Z]*EventToVerifyTheName\" in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportAsSuffix()
			{
				...
				await That(events).HasSingle().Which.IsEqualTo(ExpectedEventInfo());
				await That(events.GetDescription())
					.IsEqualTo("events with name ending with \"EventToVerifyTheNameOfIt\" in assembly").AsPrefix();
			}
			...
			public async Task ShouldSupportAsWildcard()
			{
				...
				await That(events).HasSingle().Which.IsEqualTo(ExpectedEventInfo());
				await That(events.GetDescription())
					.IsEqualTo("events with name matching \"*EventToVerifyTheNameOf*\" in assembly").AsPrefix();
			}
			...
			public async Task ShouldSupportIgnoringCase()
			{
				...
				await That(events.GetDescription())
					.IsEqualTo(
						"events with name equal to \"someeventtoverifythenameofit\" ignoring case in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportIgnoringLeadingWhiteSpace()
			{
				...
				await That(events.GetDescription())
					.IsEqualTo(
						"events with name equal to \"\\t SomeEventToVerifyTheNameOfIt\" ignoring leading white-space in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportIgnoringTrailingWhiteSpace()
			{
				...
				await That(events.GetDescription())
					.IsEqualTo(
						"events with name equal to \"SomeEventToVerifyTheNameOfIt\\t \" ignoring trailing white-space in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportUsingCustomComparer()
			{
				...
				await That(events.GetDescription())
					.IsEqualTo(
						"events with name equal to \"SOmEEventTOVErIfyThENAmEofit\" using IgnoreCaseForVocalsComparer in assembly")
					.AsPrefix();
			}
		}
	}
