		...
		public sealed class Tests
		{
			...
			public async Task ShouldFilterForTypesWithName()
			{
				...
				await That(fields.GetDescription())
					.IsEqualTo("fields with name equal to \"SomeFieldToVerifyTheNameOfIt\" in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportAsPrefix()
			{
				...
				await That(fields).HasSingle().Which.IsEqualTo(ExpectedFieldInfo());
				await That(fields.GetDescription())
					.IsEqualTo("fields with name starting with \"SomeFieldToVerifyTheNameOf\" in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportAsRegex()
			{
				...
				await That(fields).HasSingle().Which.IsEqualTo(ExpectedFieldInfo());
				await That(fields.GetDescription())
					.IsEqualTo("fields with name matching regex \"[a-zA-Z]*FieldToVerifyTheName\" in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportAsSuffix()
			{
				...
				await That(fields).HasSingle().Which.IsEqualTo(ExpectedFieldInfo());
				await That(fields.GetDescription())
					.IsEqualTo("fields with name ending with \"FieldToVerifyTheNameOfIt\" in assembly").AsPrefix();
			}
			...
			public async Task ShouldSupportAsWildcard()
			{
				...
				await That(fields).HasSingle().Which.IsEqualTo(ExpectedFieldInfo());
				await That(fields.GetDescription())
					.IsEqualTo("fields with name matching \"*FieldToVerifyTheNameOf*\" in assembly").AsPrefix();
			}
			...
			public async Task ShouldSupportIgnoringCase()
			{
				...
				await That(fields.GetDescription())
					.IsEqualTo(
						"fields with name equal to \"somefieldtoverifythenameofit\" ignoring case in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportIgnoringLeadingWhiteSpace()
			{
				...
				await That(fields.GetDescription())
					.IsEqualTo(
						"fields with name equal to \"\\t SomeFieldToVerifyTheNameOfIt\" ignoring leading white-space in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportIgnoringTrailingWhiteSpace()
			{
				...
				await That(fields.GetDescription())
					.IsEqualTo(
						"fields with name equal to \"SomeFieldToVerifyTheNameOfIt\\t \" ignoring trailing white-space in assembly")
					.AsPrefix();
			}
			...
			public async Task ShouldSupportUsingCustomComparer()
			{
				...
				await That(fields.GetDescription())
					.IsEqualTo(
						"fields with name equal to \"SOmEFieldTOVErIfyThENAmEofit\" using IgnoreCaseForVocalsComparer in assembly")
					.AsPrefix();
			}
		}
	}
