...
#pragma warning restore CA1822
		...
		public sealed class ChainingTests
		{
			...
			public async Task AtIndexFromEnd_WhenAllHaveParameterAtSpecificIndexFromEnd_ShouldSucceed()
			{
				IEnumerable<MethodInfo> methods = new[]
				{
					typeof(TestClass).GetMethod(nameof(TestClass.MethodWithIntAndString))!, // string at index 0 from end (last)
					typeof(TestClass).GetMethod(nameof(TestClass.MethodWithString))! // string at index 0 from end (last)
				};
